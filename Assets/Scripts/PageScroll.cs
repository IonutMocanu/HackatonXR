...
public class PageScroll : UIBehaviour
{
    ...
    {
        var index = pageIndex < 0 ? 0 : pageIndex > Pages.Count - 1 ? Pages.Count - 1 : pageIndex;
        if (this.PageIndex != index)
        {
            this.PageIndex = index;
            Pages[this.PageIndex].toggle.isOn = true;
        }
    }

    public void ScrollPage(int direction)
    {
        var newPage = PageIndex + direction;
    ...
    private void ActiveToggleChanged(UnityEngine.UI.Toggle toggle)
    {
        ...
        var toggleIndex = Pages.FindIndex(page => page.toggle == toggle);
        var containsToggle = toggleIndex >= 0;
        if (!containsToggle) return;
        PageIndex = toggleIndex;
    }
    ...
    private IEnumerator LateStart()
    {
        ...
        Pages[0].toggle.isOn = true;
    }

    protected virtual void Update()
    {
        _pageAnim = Mathf.Lerp(_pageAnim, (float)PageIndex, animationSpeed * Time.deltaTime);
        _pageAnim = Mathf.Clamp(_pageAnim, 0.0f, Pages.Count - 1);
        UpdateVisial();
    }

    private void UpdateVisial()
    {
        if (Mathf.Abs(_pageAnim - (float)PageIndex) < 0.005f)
        {
            var position = Pages[PageIndex].container.anchoredPosition;
            Pages[PageIndex].canvasGroup.alpha = 1.0f;
            SetOtherPagesTransparent(PageIndex, -1);
            _contentContainer.anchoredPosition = position * new Vector2(-1.0f, 1.0f);
        }
        else
        {
            var anim = Mathf.Clamp(_pageAnim, 0.0f, Pages.Count - 1);
            ...
            var startPosition = Pages[currentPageIndex].container.anchoredPosition;
            var endPosition = Pages[nextPageIndex].container.anchoredPosition;

            SetOtherPagesTransparent(currentPageIndex, nextPageIndex);
            Pages[currentPageIndex].canvasGroup.alpha = alphaTransitionCurve.Evaluate(1.0f - animParam);
            Pages[nextPageIndex].canvasGroup.alpha = alphaTransitionCurve.Evaluate(animParam);
    ...
    private void SetOtherPagesTransparent(int index0, int index1)
    {
        for (int i = 0; i < Pages.Count; i++)
        {
            ...
            if (Pages[i].canvasGroup == null) continue;
            Pages[i].canvasGroup.alpha = 0;
        }
    }
